...
<body>
	<div class="root">
		...
		<form action="" method="get" class="sign-form" autocomplete="off">
			<h2>모임 만들기</h2>
			<p>
				모임에 필요한 정보들을 설정하여 개설해보세요.<br />새로운 경험이 찾아옵니다.
			</p>
			<div class="block">
				<label>모임의 이름을 설정하세요.</label>
				<div>
					<input type="text" name="event" />
				</div>
			</div>
			<div class="block">
				<label>모임의 공개 여부를 설정하세요.</label>
				<div>
					<input id="public" type="radio" name="type" value="public" checked /><label for="public">공개</label>
					<input id="private" type="radio" name="type" value="private" /><label for="private">비공개</label>
				</div>
			</div>
			<div class="block">
				<label>모임의 목적을 설정하세요.</label>
				<div>
					<select name="cate">
						<c:forTokens items="스포츠,문화,학습,봉사,사교,게임,기타" delims="," var="one">
							<option value="${one }">${one }</option>
			...
			<div class="block">
				<label>모임을 진행할 후보 날짜기간을 선택하세요. (후보가 하나라면 같은 날로 설정하세요.)</label>
				<div>
					<input type="date"/> ~ <input type="date"/>
				</div>
			</div>
			<div class="block">
				<label>모임의 최대 참가 인원을 설정하세요.</label>
				<div>
			...
			<div class="block">
				...
				<div>
					<textarea style="resize: none; height: 160px; width: 100%"></textarea>
				</div>
			</div>
			<div class="block">
				<button>모임 개설하기</button>
			</div>
		</form>
	</div>
</body>
